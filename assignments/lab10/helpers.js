...
async function checkIsProperPassword(password, passwordName) {
    const specialChar = /["#$%&'()*+,-./:;<=>?@[\]^_`{|}~]/;
    const digitChar = /[0-9]/;
    const uppercaseChar = /[A-Z]/;
    const regex = new RegExp(/^[A-Za-z0-9"#$%&'()*+,-./:;<=>?@[\]^_`{|}~]{6,}$/);
    if(!password.match(uppercaseChar)){
        throw `Password needs to have one uppercase character and the special character cannot be space in this case`;
    }
    if(!password.match(digitChar)){
        throw `Password needs to have at least 1 digit`
    }
    if(!password.match(specialChar)){
        throw`Password needs to have at least 1 special character`
    }
    if (!regex.test(password))  {
        throw `The password needs to be at least one uppercase character, there has to be at least one number and there has to be at least one special character`;
    }
}
